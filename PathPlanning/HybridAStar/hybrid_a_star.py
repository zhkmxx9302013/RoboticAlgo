...
def analytic_expantion(current, goal, c, ox, oy, kdtree):
    ...
    paths = rs.calc_paths(sx, sy, syaw, gx, gy, gyaw, max_curvature, step_size=MOTION_RESOLUTION) # 获取rs曲线路径
...
def update_node_with_analystic_expantion(current, goal, c, ox, oy, kdtree):
    """
    计算rs路径，并更新节点cost等信息
    :param current:
    :param goal:
...
def hybrid_a_star_planning(start, goal, ox, oy, xyreso, yawreso):
    ...
    obkdtree = KDTree(np.vstack((tox, toy)).T)  # obstacle KD树
    ...
    # 解a*
    _, _, h_dp = dp_planning(nstart.xlist[-1], nstart.ylist[-1], ngoal.xlist[-1], ngoal.ylist[-1], ox, oy, xyreso, VR) # VR机器人半径
