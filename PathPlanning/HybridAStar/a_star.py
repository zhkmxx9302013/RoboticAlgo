...
def dp_planning(sx, sy, gx, gy, ox, oy, reso, rr):
    ...
    while 1:
        ...
        for i, _ in enumerate(motion):
            ...
            if n_id in closedset:   # 在close表中
                continue

            if not verify_node(node, obmap, minx, miny, maxx, maxy):    # 出界或遇到障碍
                continue

            if n_id not in openset: # 不在open表中
                openset[n_id] = node  # 加入open表进行扩展
                heapq.heappush(pq, (node.cost, calc_index(node, xw, minx, miny)))
            else:   # 在open表中
                if openset[n_id].cost >= node.cost: # 更优的路径加入openset继续扩展
                    openset[n_id] = node
                    heapq.heappush(pq, (node.cost, calc_index(node, xw, minx, miny)))
